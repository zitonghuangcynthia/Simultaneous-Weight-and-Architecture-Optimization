...
def save_dataset(MLP_type, num_hidden_layer, sparsity):
  """
    ...
    Args:
        MLP_type (str): Type of MLP (e.g., 'sigmoid', 'leakyrelu').
...
if __name__ == "__main__":
    ...
    parser.add_argument("--activation", type=str, required=True, help="Type of MLP (sigmoid, leakyrelu, linear).")
    ...
    if args.activation.lower() not in valid_mlp_types:
        raise ValueError(f"Invalid MLP_type '{args.activation}'. Supported values are {valid_mlp_types}.")
    ...
    save_dataset(args.activation, args.num_hidden_layer, args.sparsity)
